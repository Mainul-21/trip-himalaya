...
const trustPoints = [
  [MapPinned, "Local Experts", "Born in the Himalayas"],
  [BadgeIndianRupee, "Best Price Guarantee", "No hidden charges"],
  [ShieldCheck, "Safe & Comfortable", "Your safety, our priority"],
  [Headphones, "24x7 Support", "We are always with you"],
] as const;
...
export default function Home() {
  ...
  return <PublicLayout>
    <section className="relative isolate min-h-[660px] overflow-hidden bg-[#0e3d5c] text-white sm:min-h-[700px]">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(7,39,61,.9)_0%,rgba(7,39,61,.62)_55%,rgba(7,39,61,.24)_100%)]" />
      <div className="container relative flex min-h-[660px] flex-col justify-center pb-28 pt-36 sm:min-h-[700px]">
        <div className="max-w-2xl">
         
          <h1 className="rise-2 mt-4 font-display text-[clamp(3.1rem,6.4vw,5.7rem)] font-bold leading-[.9] tracking-[-.045em]">Discover Himachal.<span className="mt-2 block text-[#f39a48]">Experience the Himalayas.</span></h1>
        ...
        <form onSubmit={search} className="rise-3 mt-9 grid max-w-2xl gap-2 border border-white/24 bg-[#0b3553]/92 p-2 sm:grid-cols-[1fr_auto]" role="search">
          <label className="sr-only" htmlFor="hero-search">Search trips</label>
          <div className="flex items-center gap-3 px-3"><Search className="size-5 shrink-0 text-[#f5a653]" /><input id="hero-search" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search Triund, Bir, Dharamshala or a trip type" className="h-11 min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/58" /></div>
    ...
    <section className="py-16 sm:py-20"><div className="container"><div className="grid gap-5 lg:grid-cols-[1fr_.68fr] lg:items-end"><div><p className="eyebrow">Travel styles</p><h2 className="section-title mt-3">{agency?.exploreTitle ?? "Choose your travel style."}</h2><p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">{agency?.exploreIntro ?? "Pick a style to find a journey that fits."}</p></div><Link href="/tours" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-[#123d5b] hover:text-[#e17818] lg:justify-self-end">Browse all journeys <ArrowRight className="size-4" /></Link></div><div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{visibleTripStyles.map(item => <Link key={item.title} href={item.href} className="group relative isolate h-52 overflow-hidden rounded-xl border border-[#dfe8e8] bg-[#123d5b] shadow-[0_10px_24px_rgba(18,61,91,.08)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#e9781c] focus-visible:ring-offset-4"><img src={getImageVariant(item.image, "card")} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" loading="lazy" fetchPriority="low" decoding="async" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" /><div className="absolute inset-0 bg-gradient-to-t from-[#082f4b]/96 via-[#082f4b]/24 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-5 text-white"><h3 className="font-display text-2xl font-bold leading-none">{item.title}</h3><p className="mt-2 text-sm leading-5 text-white/75">{item.copy}</p></div></Link>)}</div></div></section>

    <section className="border-y border-[#e5ece7] bg-[#fbfaf6] py-16 sm:py-20"><div className="container"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><h2 className="section-title mt-3">Featured journeys</h2><p className="mt-3 max-w-lg text-sm leading-6 text-slate-600">Four current Himachal journeys selected and ordered by the Trip Himalaya team.</p></div><Link href="/tours" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-[#123d5b] hover:text-[#e17818]">See all journeys <ArrowRight className="size-4" /></Link></div><div className="mt-9 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{topTours.length ? topTours.map(tour => <TourCard key={tour.id} tour={tour} />) : <div className="col-span-full border border-dashed border-[#ccd9d7] p-10 text-center text-sm text-slate-500">New journeys are being prepared. Please check back shortly.</div>}</div></div></section>

    <section className="border-y border-[#dbe6e3] bg-[#f7faf8] py-16 sm:py-20"><div className="container"><div className="mx-auto max-w-2xl text-center"><h2 className="section-title mt-3">Why choose Trip Himalaya</h2><p className="mt-4 text-sm leading-6 text-slate-600">Practical details and a real person to help before the journey begins.</p></div><div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{whyChoosePoints.map(([Icon, title, copy]) => <article key={String(title)} className="border border-[#d9e5e0] bg-white p-5 text-center"><span className="mx-auto grid size-11 place-items-center rounded-md bg-[#123d5b] text-[#f6a85c]"><Icon className="size-5" /></span><h3 className="mt-4 text-sm font-extrabold uppercase tracking-[.08em] text-[#123d5b]">{String(title)}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{String(copy)}</p></article>)}</div></div></section>
