  ...
  return <PublicLayout>
    <section className="relative isolate overflow-hidden bg-[#0a3450] text-white">
      <div className="relative min-h-[540px] overflow-hidden sm:min-h-[585px]">
        ...
        <div className="container relative flex min-h-[540px] flex-col justify-center pb-20 pt-28 sm:min-h-[585px] sm:pb-24 sm:pt-32">
          <div className="max-w-3xl"><p className="rise-1 text-[.68rem] font-extrabold uppercase tracking-[.2em] text-[#ffd09b]">Dharamshala · Himachal Pradesh</p><h1 className="rise-2 mt-4 font-display text-[clamp(2.85rem,5.6vw,5.45rem)] font-bold leading-[.87] tracking-[-.055em] text-white">Discover Himachal.<span className="mt-2 block text-[#ff951e]">Experience the Himalayas.</span></h1><p className="rise-3 mt-5 max-w-xl text-[.95rem] font-medium leading-7 text-white/88 sm:text-lg">Thoughtful journeys, local knowledge and a clear plan for the mountains.</p></div>
          <div className="rise-3 mt-7 flex flex-wrap gap-3"><Link href="/tours" className="focus-ring inline-flex h-11 items-center gap-2 rounded-sm bg-[#ef7916] px-5 text-[.72rem] font-extrabold uppercase tracking-[.04em] text-white shadow-[0_8px_20px_rgba(0,0,0,.16)] transition hover:bg-[#d9690b] active:scale-[.98]">Explore tours <ArrowRight className="size-4" /></Link><Link href="/contact" className="focus-ring inline-flex h-11 items-center gap-2 rounded-sm bg-white px-5 text-[.72rem] font-extrabold uppercase tracking-[.04em] text-[#123d5b] transition hover:bg-[#f7f1e8] active:scale-[.98]">Plan your trip <CalendarDays className="size-4" /></Link></div>
          <div className="absolute bottom-6 left-4 right-4 flex items-center justify-between sm:left-6 sm:right-6 lg:left-10 lg:right-10"><button type="button" onClick={() => setActiveHeroSlide(current => current === 0 ? heroSlides.length - 1 : current - 1)} className="focus-ring grid size-10 place-items-center rounded-full border border-white/55 bg-[#0a3450]/30 text-white transition hover:bg-white hover:text-[#123d5b]" aria-label="Show previous hero image"><ArrowLeft className="size-5" /></button><div className="flex gap-2" aria-label="Hero image selection">{heroSlides.map((slide, index) => <button key={slide.src} type="button" onClick={() => setActiveHeroSlide(index)} className={`focus-ring h-1.5 rounded-full transition ${index === activeHeroSlide ? "w-8 bg-[#ff951e]" : "w-4 bg-white/55 hover:bg-white"}`} aria-label={`Show hero image ${index + 1}`} aria-current={index === activeHeroSlide ? "true" : undefined} />)}</div><button type="button" onClick={() => setActiveHeroSlide(current => getNextHeroSlideIndex(current))} className="focus-ring grid size-10 place-items-center rounded-full border border-white/55 bg-[#0a3450]/30 text-white transition hover:bg-white hover:text-[#123d5b]" aria-label="Show next hero image"><ArrowRight className="size-5" /></button></div>
        </div>
      </div>
