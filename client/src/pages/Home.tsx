...
export default function Home() {
  ...
  return <PublicLayout showHeader={false}>
    ...
    {reviews.length || reviewCtaUrl ? <section className="bg-[linear-gradient(180deg,#f8fbfa_0%,#ffffff_100%)] pb-16 pt-2" aria-labelledby="traveller-stories-title"><div className="mx-auto max-w-7xl px-5"><div className="overflow-hidden rounded-2xl border border-[#dbe8e4] bg-card shadow-[0_22px_60px_rgba(18,61,91,.10)]"><div className="grid lg:grid-cols-[.9fr_2.1fr]"><aside className="relative overflow-hidden bg-primary p-7 text-primary-foreground sm:p-9"><div className="absolute -right-16 -top-20 h-48 w-48 rounded-full border border-primary-foreground/10" /><div className="relative"><span className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/20 bg-primary-foreground/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[.16em] text-primary-foreground"><CheckCircle2 size={13} className="text-accent" /> Traveller feedback</span><h2 id="traveller-stories-title" className="mt-5 font-display text-2xl font-bold tracking-wide">{reviewSectionTitle}</h2><p className="mt-3 max-w-sm text-sm leading-6 text-primary-foreground/78">{reviewSectionIntro}</p><p className="mt-6 rounded-xl border border-primary-foreground/15 bg-primary-deep/30 p-4 text-sm leading-6 text-primary-foreground/78">Read guest stories below or use the provided feedback source to learn more about Trip Himalaya journeys.</p>{reviewCtaUrl ? <a href={reviewCtaUrl} target="_blank" rel="noreferrer" className="focus-ring mt-6 inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-3 text-xs font-bold tracking-wide text-accent-foreground transition-transform hover:scale-[1.02]">{reviewCtaLabel} <ExternalLink size={15} /></a> : <Link href="/contact" className="focus-ring mt-6 inline-flex items-center gap-2 rounded-lg bg-background px-4 py-3 text-xs font-bold tracking-wide text-primary transition-transform hover:scale-[1.02]">CONTACT TRIP HIMALAYA <ChevronRight size={15} /></Link>}<p className="mt-4 text-[10px] leading-5 text-primary-foreground/55">Review links and displayed guest feedback are controlled by Trip Himalaya administration.</p></div></aside><div className="p-5 sm:p-7"><div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-accent">Guest notes</p><h3 className="mt-1 font-display text-xl font-bold tracking-wide text-primary">WHAT OUR TRAVELLERS SAY</h3></div>{reviewCtaUrl ? <a href={reviewCtaUrl} target="_blank" rel="noreferrer" className="focus-ring hidden items-center gap-1.5 text-xs font-semibold text-primary underline decoration-accent underline-offset-4 sm:inline-flex">{reviewCtaLabel} <ExternalLink size={13} /></a> : null}</div>{reviews.length ? <div className="grid gap-4 md:grid-cols-3">{reviews.slice(0, 3).map(item => <article key={item.id} className="group rounded-xl border border-[#e2ece8] bg-white p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-accent/40 hover:shadow-[0_12px_30px_rgba(18,61,91,.08)]"><div className="flex items-start justify-end"><Quote size={22} className="text-accent/65" aria-hidden="true" /></div><p className="mt-4 line-clamp-5 text-sm leading-6 text-slate-600">“{item.quote}”</p><div className="mt-5 flex items-center gap-3 border-t border-[#edf2f0] pt-4"><span className="grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-[#edf4f1] text-xs font-extrabold text-primary">{item.reviewerImage ? <img src={item.reviewerImage} alt="" className="h-full w-full object-cover" /> : item.reviewerName.charAt(0)}</span><div className="min-w-0"><p className="truncate text-xs font-bold text-primary">{item.reviewerName}</p><p className="truncate text-[10px] text-muted-foreground">{item.location || item.sourceLabel || "Guest feedback"}</p></div></div></article>)}</div> : <div className="rounded-xl border border-dashed border-[#c7d8d2] bg-[#f8fbfa] p-7 text-center"><p className="text-sm font-semibold text-primary">Guest stories will appear here.</p><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-muted-foreground">Trip Himalaya publishes feedback only after the administration team has received and approved it.</p></div>}</div></div></div></div></section> : null}
