...
const whyChoosePoints = [
  [MapPinned, "Local Dharamshala planning", "Get route, stay and timing guidance from a team based in Himachal."],
  [Compass, "Practical trip details", "Understand the pace, season and what is included before you decide."],
  [UsersRound, "A plan for your group", "We shape the starting point around your dates, group and travel style."],
];
...
export default function Home() {
  ...
  return <PublicLayout>
    ...
    <section className="py-16 sm:py-20"><div className="container"><div className="grid gap-5 lg:grid-cols-[1fr_.68fr] lg:items-end"><div><p className="eyebrow">Find your way into Himachal</p><h2 className="section-title mt-3">Choose a trip style.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">Start with the kind of experience you want. You can refine your choice again on the journeys page.</p></div><Link href="/tours" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-[#123d5b] hover:text-[#e17818] lg:justify-self-end">Browse all journeys <ArrowRight className="size-4" /></Link></div><div className="mt-8 grid gap-px overflow-hidden border border-[#dfe8e8] bg-[#dfe8e8] sm:grid-cols-2 lg:grid-cols-3">{tripStyles.map(item => <Link key={item.title} href={item.href} className="group relative isolate h-52 overflow-hidden bg-[#123d5b] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#e9781c] focus-visible:ring-offset-4"><img src={item.image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-t from-[#082f4b]/94 via-[#082f4b]/18 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-5 text-white"><h3 className="font-display text-2xl font-bold leading-none">{item.title}</h3><p className="mt-2 text-sm leading-5 text-white/75">{item.copy}</p></div></Link>)}</div></div></section>
    ...
    <section className="py-16 sm:py-20"><div className="container grid gap-12 lg:grid-cols-[.92fr_1.08fr] lg:items-center"><img src="/manus-storage/dharamshala-prayer-flags_26329188.jpg" alt="Prayer flags in Dharamshala" className="h-[335px] w-full object-cover sm:h-[390px]" loading="lazy" /><div><p className="eyebrow">Why choose Trip Himalaya</p><h2 className="section-title mt-3 max-w-xl">Clear plans for better mountain time.</h2><p className="mt-5 max-w-xl text-base leading-7 text-slate-600">We keep travel planning simple: choose the right route, understand the practical details, and speak to someone before you commit.</p><div className="mt-8 divide-y divide-[#dfe8e8] border-y border-[#dfe8e8]">{whyChoosePoints.map(([Icon, title, copy]) => <div key={String(title)} className="flex gap-4 py-4"><Icon className="mt-0.5 size-5 shrink-0 text-[#e17818]" /><span><strong className="text-sm text-[#123d5b]">{String(title)}</strong><p className="mt-1 text-sm leading-6 text-slate-500">{String(copy)}</p></span></div>)}</div></div></div></section>
