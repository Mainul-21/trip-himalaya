...
const whyUs = [[Mountain, "LOCAL EXPERTS", "We are locals, we know the Himalayas best."], [ShieldCheck, "SAFE & RELIABLE", "Your safety and comfort is our top priority."], [BadgeIndianRupee, "BEST PRICE GUARANTEE", "Transparent pricing with no hidden charges."], [Users, "PERSONALISED SUPPORT", "From planning to journey, we are with you."], [Leaf, "RESPONSIBLE TOURISM", "We respect nature & support local communities."]] as const;
...
export default function Home() {
  ...
  useEffect(() => { if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; const timer = window.setInterval(() => setActiveHeroSlide(current => getNextHeroSlideIndex(current)), 4000); return () => window.clearInterval(timer); }, []);
