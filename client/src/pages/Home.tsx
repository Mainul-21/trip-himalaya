...
export default function Home() {
  ...
  return <PublicLayout>
    ...
    <section className="py-16 sm:py-20"><div className="container"><div className="grid gap-5 lg:grid-cols-[1fr_.68fr] lg:items-end"><div><p className="eyebrow">Explore Himachal</p><h2 className="section-title mt-3">Choose your travel style.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">Start with the kind of mountain time you want. Every choice opens a filtered journey list.</p></div><Link href="/tours" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-[#123d5b] hover:text-[#e17818] lg:justify-self-end">Browse all journeys <ArrowRight className="size-4" /></Link></div><div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{tripStyles.map(item => <Link key={item.title} href={item.href} className="group relative isolate h-52 overflow-hidden rounded-xl border border-[#dfe8e8] bg-[#123d5b] shadow-[0_10px_24px_rgba(18,61,91,.08)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#e9781c] focus-visible:ring-offset-4"><img src={item.image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-t from-[#082f4b]/96 via-[#082f4b]/24 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-5 text-white"><h3 className="font-display text-2xl font-bold leading-none">{item.title}</h3><p className="mt-2 text-sm leading-5 text-white/75">{item.copy}</p></div></Link>)}</div></div></section>
