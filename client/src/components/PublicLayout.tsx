  ...
  return <header className={`relative z-50 isolate ${isHomepage ? "" : "bg-primary"}`}>
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-4">
      ...
      <div className="flex items-center gap-2 sm:gap-3 lg:contents">
        {!isHomepage ? <a href={phoneHref} className="focus-ring hidden items-center gap-2 rounded-full border border-primary-foreground/30 px-4 py-2.5 text-xs font-semibold tracking-wide text-primary-foreground transition-colors hover:border-accent hover:text-accent xl:inline-flex"><Phone size={14} /> CALL NOW</a> : null}
        <Link href={planHref} className="focus-ring hidden items-center gap-2 rounded-full bg-accent px-5 py-2.5 text-xs font-semibold tracking-wide text-accent-foreground shadow-md transition-transform hover:scale-[1.03] md:inline-flex">PLAN YOUR TRIP <span className="grid h-5 w-5 place-items-center rounded-full border border-accent-foreground/60"><CalendarDays size={11} /></span></Link>
        <button type="button" className="focus-ring grid h-11 w-11 place-items-center rounded-full border border-primary-foreground/40 text-primary-foreground transition-colors hover:bg-primary-foreground/10 lg:hidden" onClick={() => setMobileOpen(open => !open)} aria-expanded={mobileOpen} aria-controls="mobile-menu" aria-label={mobileOpen ? "Close navigation menu" : "Open navigation menu"}>{mobileOpen ? <X size={20} /> : <Menu size={21} strokeWidth={2.3} />}</button>
      </div>
    </div>
