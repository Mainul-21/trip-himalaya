...
export function PublicHeader({ profile = fallbackAgencyProfile }: { profile?: AgencyProfile }) {
  ...
  return <header className="relative z-50 isolate border-b border-[#0D2C5B]/10 bg-white/95 shadow-[0_8px_24px_rgba(13,44,91,.1)] backdrop-blur-xl">
    <div className="hidden border-b border-white/10 bg-[#0D2C5B] text-white lg:block">
      <div className="mx-auto flex h-9 max-w-7xl items-center justify-between gap-4 px-5 text-[10px] font-semibold tracking-wide">
        <div className="flex items-center gap-5 text-white/90"><span className="flex items-center gap-1.5"><MapPin size={13} className="text-[#F56600]" /> Based in Himachal Pradesh</span><a href={phoneHref} className="flex items-center gap-1.5 transition-colors hover:text-[#F56600]"><Phone size={12} className="text-[#F56600]" /> {profile.phone}</a><a href={`mailto:${profile.email}`} className="flex items-center gap-1.5 transition-colors hover:text-[#F56600]"><Mail size={12} className="text-[#F56600]" /> {profile.email}</a></div>
        <div className="flex items-center gap-4 text-white/90">{profile.instagramUrl ? <a href={profile.instagramUrl} target="_blank" rel="noreferrer" aria-label="Instagram" className="transition-colors hover:text-[#F56600]"><Instagram size={17} /></a> : null}{profile.facebookUrl ? <a href={profile.facebookUrl} target="_blank" rel="noreferrer" aria-label="Facebook" className="transition-colors hover:text-[#F56600]"><Facebook size={16} /></a> : null}<a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noreferrer" aria-label="WhatsApp" className="transition-colors hover:text-[#F56600]"><WhatsAppIcon className="size-[17px]" /></a></div>
      </div>
    </div>
    <div className="mx-auto flex min-h-[4.25rem] max-w-7xl items-center justify-between gap-3 px-4 py-1.5 lg:min-h-[5.25rem] lg:gap-5 lg:px-6">
      <Link href="/" className="focus-ring flex w-fit shrink-0 items-center" aria-label={`${profile.brandName} home`}><img src={SUPPLIED_COMPLETE_HEADER_LOGO} alt={`${profile.brandName} logo`} width={210} height={77} className="h-auto w-[138px] object-contain sm:w-[162px] lg:w-[210px]" /></Link>
      <nav className="hidden items-center gap-4 xl:gap-6 lg:flex" aria-label="Main navigation">
        {navigation.map(link => <Link key={link.href} href={link.href} className={`focus-ring flex items-center gap-1 text-[11px] font-bold tracking-wide text-[#0D2C5B] transition-colors hover:text-[#F56600] ${location.split("?")[0] === link.href ? "border-b-2 border-[#F56600] pb-1 text-[#0D2C5B]" : ""}`}>{link.label}{link.dropdown ? <ChevronDown size={12} /> : null}</Link>)}
        {user ? <Link href="/admin" className="focus-ring flex shrink-0 items-center whitespace-nowrap border-b-2 border-transparent pb-1 text-[11px] font-bold tracking-wide text-[#0D2C5B] transition-colors hover:text-[#F56600]">ADMIN DASHBOARD</Link> : null}
      </nav>
      <div className="flex items-center gap-2 sm:gap-3 lg:contents">
        <Link href={planHref} className="focus-ring hidden items-center gap-1.5 rounded-md bg-[#F56600] px-5 py-2.5 text-[11px] font-bold tracking-wide text-white shadow-md transition-transform hover:scale-[1.03] hover:bg-[#d95600] md:inline-flex">PLAN YOUR TRIP <span className="grid h-4 w-4 place-items-center rounded-full border border-white/70"><Send size={9} /></span></Link>
        <button type="button" className="focus-ring grid h-10 w-10 place-items-center rounded-md border border-[#0D2C5B]/25 text-[#0D2C5B] transition-colors hover:bg-[#0D2C5B]/5 lg:hidden" onClick={() => setMobileOpen(open => !open)} aria-expanded={mobileOpen} aria-controls="mobile-menu" aria-label={mobileOpen ? "Close navigation menu" : "Open navigation menu"}>{mobileOpen ? <X size={19} /> : <Menu size={20} strokeWidth={2.3} />}</button>
      </div>
    </div>
    {mobileOpen ? <div id="mobile-menu" className="absolute inset-x-0 top-full z-50 max-h-[calc(100dvh-4.25rem)] overflow-y-auto border-t border-[#0D2C5B]/10 bg-white px-5 py-4 shadow-2xl lg:hidden"><nav className="mx-auto grid max-w-7xl gap-1" aria-label="Mobile navigation">{navigation.map(link => <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className="focus-ring flex min-h-11 items-center rounded-md px-3 py-3 text-sm font-bold leading-5 tracking-wide text-[#0D2C5B] hover:bg-[#0D2C5B]/5">{link.label}</Link>)}{user ? <Link href="/admin" onClick={() => setMobileOpen(false)} className="focus-ring flex min-h-11 items-center rounded-md px-3 py-3 text-sm font-bold leading-5 tracking-wide text-[#0D2C5B] hover:bg-[#0D2C5B]/5">ADMIN DASHBOARD</Link> : null}<div className={`mt-3 grid gap-2 ${isHomepage ? "sm:grid-cols-2" : "sm:grid-cols-3"}`}>{!isHomepage ? <a href={phoneHref} className="focus-ring flex min-h-11 items-center justify-center gap-2 rounded-full border border-[#0D2C5B]/25 px-3 py-3 text-center text-xs font-bold leading-5 tracking-wide text-[#0D2C5B]"><Phone size={14} /> CALL NOW</a> : null}<a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent("Hello, I would like to know more about your Dharamshala tour packages.")}`} target="_blank" rel="noreferrer" className="focus-ring flex min-h-11 items-center justify-center gap-2 rounded-full border border-[#0D2C5B]/25 px-3 py-3 text-center text-xs font-bold leading-5 tracking-wide text-[#0D2C5B]"><WhatsAppIcon className="size-4" /> WHATSAPP</a><Link href={planHref} onClick={() => setMobileOpen(false)} className="focus-ring flex min-h-11 items-center justify-center rounded-full bg-[#F56600] px-3 py-3 text-center text-xs font-bold leading-5 tracking-wide text-white">PLAN YOUR TRIP</Link></div></nav></div> : null}
  </header>;
}
