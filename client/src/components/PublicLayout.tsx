...
export function PublicHeader({ profile = fallbackAgencyProfile }: { profile?: AgencyProfile }) {
  ...
  return <header className="relative z-50 isolate border-b border-[#0D2C5B]/10 bg-white/95 shadow-[0_8px_24px_rgba(13,44,91,.1)] backdrop-blur-xl">
    ...
    <div className="mx-auto flex min-h-[4.75rem] max-w-7xl items-center justify-between gap-4 px-5 py-2.5 lg:min-h-[6.25rem] lg:gap-6 lg:px-7">
      <Link href="/" className="focus-ring flex w-fit shrink-0 items-center" aria-label={`${profile.brandName} home`}><img src={SUPPLIED_COMPLETE_HEADER_LOGO} alt={`${profile.brandName} logo`} width={232} height={85} className="h-auto w-[150px] object-contain sm:w-[178px] lg:w-[232px]" /></Link>
