...
export function PublicHeader({ profile = fallbackAgencyProfile }: { profile?: AgencyProfile }) {
  ...
  return <header className="relative z-50 isolate border-b border-[#0D2C5B]/10 bg-white/95 shadow-[0_8px_24px_rgba(13,44,91,.1)] backdrop-blur-xl">
    <div className="hidden border-b border-white/10 bg-[#0D2C5B] text-white lg:block">
      <div className="mx-auto flex h-8 max-w-7xl items-center justify-between gap-4 px-5 text-[9px] font-semibold tracking-wide">
        <div className="flex items-center gap-5 text-white/90"><span className="flex items-center gap-1"><MapPin size={12} className="text-[#F56600]" /> Based in Himachal Pradesh</span><a href={phoneHref} className="flex items-center gap-1 transition-colors hover:text-[#F56600]"><Phone size={11} className="text-[#F56600]" /> {profile.phone}</a><a href={`mailto:${profile.email}`} className="flex items-center gap-1 transition-colors hover:text-[#F56600]"><Mail size={11} className="text-[#F56600]" /> {profile.email}</a></div>
