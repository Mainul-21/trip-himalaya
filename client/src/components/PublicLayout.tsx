  ...
  return <div className="min-h-screen overflow-hidden bg-[#fbfaf6]">
    <header className="sticky top-0 z-50 border-b border-[#dbe5e9]/75 bg-[#fbfaf6]/93 backdrop-blur-lg">
      <div className="container flex h-[74px] items-center justify-between gap-4">
        ...
        <div className="hidden items-center gap-3 lg:flex"><SearchField /><Link href="/contact" className="focus-ring inline-flex h-10 items-center gap-2 rounded-xl bg-[#e9781c] px-4 text-[.72rem] font-extrabold uppercase tracking-[.09em] text-white shadow-[0_8px_18px_rgba(232,120,28,.22)] transition-transform active:scale-[.97] hover:bg-[#d86b12]"><MessageCircle className="size-4" /> Plan your trip</Link></div>
        <button type="button" className="focus-ring grid size-10 shrink-0 place-items-center rounded-xl border border-[#dbe5e9] text-[#123d5b] lg:hidden" onClick={() => setMobileOpen(open => !open)} aria-expanded={mobileOpen} aria-controls="mobile-menu" aria-label={mobileOpen ? "Close navigation menu" : "Open navigation menu"}>{mobileOpen ? <X /> : <Menu />}</button>
      </div>
      {mobileOpen && <div id="mobile-menu" className="fixed inset-x-0 top-[74px] z-50 max-h-[calc(100dvh-74px)] overflow-y-auto border-t border-[#dbe5e9] bg-[#fbfaf6] px-4 py-5 shadow-2xl lg:hidden"><nav className="container grid gap-1" aria-label="Mobile navigation">{navigation.map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileOpen(false)} className="focus-ring rounded-lg px-3 py-3 text-sm font-bold text-[#123d5b] hover:bg-[#eef4f2]">{label}</Link>)}<div className="mt-3 grid grid-cols-2 gap-2"><a className="focus-ring inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-[#c8d9d6] text-xs font-extrabold uppercase tracking-[.08em] text-[#123d5b]" href="tel:+919999999999"><Phone className="size-4" /> Call us</a><a className="focus-ring inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#28b56b] text-xs font-extrabold uppercase tracking-[.08em] text-white" href="https://wa.me/919999999999" target="_blank" rel="noreferrer"><MessageCircle className="size-4" /> WhatsApp</a></div><Link href="/contact" onClick={() => setMobileOpen(false)} className="focus-ring mt-2 rounded-xl bg-[#e9781c] px-4 py-3 text-center text-xs font-extrabold uppercase tracking-[.1em] text-white">Plan your trip</Link></nav></div>}
    </header>
    {children}
    <FloatingContact />
