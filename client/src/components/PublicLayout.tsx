...
export function PublicHeader({ profile = fallbackAgencyProfile }: { profile?: AgencyProfile }) {
  ...
  return <header className={`relative z-10 ${isHomepage ? "" : "bg-primary"}`}>
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-4">
      ...
      <button type="button" className="focus-ring grid h-10 w-10 place-items-center rounded-md border border-primary-foreground/40 text-primary-foreground md:hidden" onClick={() => setMobileOpen(open => !open)} aria-expanded={mobileOpen} aria-controls="mobile-menu" aria-label={mobileOpen ? "Close navigation menu" : "Open navigation menu"}>{mobileOpen ? <X size={19} /> : <Menu size={19} />}</button>
    </div>
    {mobileOpen ? <div id="mobile-menu" className="absolute inset-x-0 top-full border-t border-primary-foreground/15 bg-primary-deep px-5 py-4 shadow-lg md:hidden"><nav className="mx-auto grid max-w-7xl gap-1" aria-label="Mobile navigation">{navigation.map(link => <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className="focus-ring rounded-md px-3 py-3 text-sm font-semibold tracking-wide text-primary-foreground hover:bg-primary-foreground/10">{link.label}</Link>)}{user ? <Link href="/admin" onClick={() => setMobileOpen(false)} className="focus-ring rounded-md px-3 py-3 text-sm font-semibold tracking-wide text-primary-foreground hover:bg-primary-foreground/10">ADMIN DASHBOARD</Link> : null}<Link href={isHomepage ? "#plan" : "/contact"} onClick={() => setMobileOpen(false)} className="focus-ring mt-2 rounded-md bg-accent px-3 py-3 text-center text-xs font-semibold tracking-wide text-accent-foreground">PLAN YOUR TRIP</Link></nav></div> : null}
  </header>;
}
